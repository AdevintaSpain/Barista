...
./google-cloud-sdk/bin/gcloud beta test android run --type instrumentation --app ./sample/build/outputs/apk/sample-debug.apk --test ./sample/build/outputs/apk/sample-debug-androidTest-unaligned.apk --device-ids Nexus5 --os-version-ids 22 --locales en --orientations portrait --project ${PROJECT_ID}  --results-history-name "${HISTORY_NAME}"
