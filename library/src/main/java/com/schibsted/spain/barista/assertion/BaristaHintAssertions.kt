...
object BaristaHintAssertions {
    ...
    fun assertHint(@IdRes viewId: Int, @StringRes text: Int) {
        val string = InstrumentationRegistry.getTargetContext().resources.getString(text)
        withId(viewId).assertAny(withLayoutHint(string))
    }

    @JvmStatic
    fun assertHint(@IdRes viewId: Int, text: String) {
        withId(viewId).assertAny(withLayoutHint(text))
    }

    private fun withLayoutHint(expectedHint: String): Matcher<View> {
